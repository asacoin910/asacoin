...
*Setup instructions for a gitian build of ASA Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the ASA
...
to the ASA GitHub release page.
...
- [Building ASA Core](#building-asacoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/asacoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/asacoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/asacoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/asacoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/asacoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/asacoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/asacoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@asacoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/asacoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/asacoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/asacoin-osx-*-build.assert
```
