...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::ASA);
    ...
    if (settings.contains("nAnonymizeAsaAmount") || reset)
        SoftSetArg("-anonymizeasacoinamount", settings.value("nAnonymizeAsaAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in asacoin.cpp during shotdown to also remove the window positions
