...
#include "qt/asacoin/dashboardwidget.h"
#include "qt/asacoin/forms/ui_dashboardwidget.h"
#include "qt/asacoin/sendconfirmdialog.h"
#include "qt/asacoin/txrow.h"
...
DashboardWidget::DashboardWidget(ASAGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of ASA earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 ASA");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &ASAGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &ASAGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &ASAGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("ASA")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair ASA, zASA
