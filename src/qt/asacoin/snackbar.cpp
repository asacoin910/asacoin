...
#include "qt/asacoin/forms/ui_snackbar.h"
#include "qt/asacoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(ASAGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &ASAGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
