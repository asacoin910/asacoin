...
#if defined(NDEBUG)
#error "ASA cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uASA) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("asacoin-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        const bool hasASAInputs = !bibInputs.empty();
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // ASA: We use certain sporks during IBD, so check to see if they are
